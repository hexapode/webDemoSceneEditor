...
const defaultNodes = [
  { id:"TXT_00", name:"Hahahack Titles", type:"TEXT OPERATOR", kind:"Text", x:20,y:4,color:"#ff3b8d", value:"6 ACTS", enabled:true,params:{text:"HAHAHACK",textColor:"#fff3b0",textSize:82,textX:50,textY:48,textRotation:-2,textWave:.42,sequence:["HAHAHACK","ABSURD MACHINE","CODEX MADE THIS","MAKE IT WEIRD","SHIP THE CHAOS","HAHAHACK!"]},bindings:{textWave:{source:"bar",amount:.22}} },
  { id:"SIG_01", name:"Beat Pulse", type:"SIGNAL OPERATOR", kind:"Beat", x:165,y:4,color:"#55d887", value:"132 BPM", enabled:true },
  { id:"GEO_02", name:"Torus Field", type:"GEOMETRY OPERATOR", kind:"Primitive", x:310,y:4,color:"#5da7ff", value:"4 SHAPES", enabled:true },
  { id:"MOD_03", name:"Act Morph", type:"MODIFIER OPERATOR", kind:"Twist", x:455,y:4,color:"#25c9cd", value:"6 × 10 SEC", enabled:true,bindings:{twist:{source:"half",amount:.18}} },
  { id:"MAT_04", name:"Candy Chrome", type:"MATERIAL OPERATOR", kind:"Iridescent", x:165,y:86,color:"#ff6d42", value:"AUDIO REACTIVE", enabled:true,bindings:{pulse:{source:"beat",amount:.32},roughness:{source:"envelope",amount:-.12}} },
  { id:"CAM_06", name:"Auto Orbit", type:"SCENE OPERATOR", kind:"Camera", x:310,y:86,color:"#f1b85b", value:"60 SEC PATH", enabled:true },
  { id:"OUT_07", name:"Hahahack Output", type:"OUTPUT OPERATOR", kind:"Output", x:600,y:86,color:"#ff5a36", value:"01:00 LOOP", enabled:true }
];
const defaultConnections = [["TXT_00","OUT_07"],["SIG_01","MOD_03"],["GEO_02","MOD_03"],["MOD_03","MAT_04"],["MAT_04","OUT_07"],["CAM_06","OUT_07"]];
...
const defaultAudioNodes=[
  {id:"AUDIO_01",name:"Beat Clock",type:"AUDIO CONTROL",kind:"Beat",x:35,y:58,color:"#55d887",value:"132 BPM",enabled:true},
...
const paramDefinitions = {
  ...
  "Grid Array":[["Columns","texKaleido",2,16,1],["Spacing","scale",.4,2,.01],["Offset","twist",0,3,.01]],Fractal:[["Iterations","texKaleido",2,16,1],["Complexity","texWarp",0,3,.01],["Scale","texScale",.5,10,.05]],
...
function sourceSignal(source){const beatPhase=state.time*state.bpm/60,phase=value=>value-Math.floor(value);if(source==="beat")return .5+.5*Math.sin(beatPhase*Math.PI*2);if(source==="half")return .5+.5*Math.sin(beatPhase*Math.PI);if(source==="bar")return .5+.5*Math.sin(beatPhase*Math.PI/2);if(source==="sine")return Math.sin(state.time*Math.PI*.5);if(source==="envelope")return Math.exp(-phase(beatPhase)*6);if(source==="noise")return Math.sin(Math.floor(state.time*12.5)*91.73)*.5+.5;if(source==="act")return Math.exp(-(state.time%10)*5);if(source==="event")return Math.exp(-(performance.now()/1000-lastExternalEvent)*8);return 0;}
...
const showActs=[
  {shapeMode:0,color:"#ff3b8d",accent:"#5cf5ff",twist:1.3,scale:1,bloom:.85,vignette:.16,exposure:1.18,pulse:.72,texScale:4.4,texSpeed:.42,texWarp:1.2,texContrast:1.6,texKaleido:7,texMix:.8,roughness:.2},
  {shapeMode:1,color:"#6b5cff",accent:"#69ffb7",twist:2.5,scale:.86,bloom:1.15,vignette:.3,exposure:1.28,pulse:.92,texScale:7.2,texSpeed:.76,texWarp:2.2,texContrast:2.3,texKaleido:9,texMix:.94,roughness:.06},
  {shapeMode:2,color:"#ffb000",accent:"#ff315f",twist:.55,scale:1.12,bloom:.96,vignette:.22,exposure:1.12,pulse:1,texScale:2.5,texSpeed:.24,texWarp:2.8,texContrast:1.25,texKaleido:5,texMix:.88,roughness:.34},
  {shapeMode:3,color:"#e9f7ff",accent:"#2779ff",twist:.18,scale:.92,bloom:.42,vignette:.52,exposure:.82,pulse:.3,texScale:5.1,texSpeed:.15,texWarp:.55,texContrast:2.5,texKaleido:4,texMix:.58,roughness:.28},
  {shapeMode:0,color:"#d5ff28",accent:"#ff23ba",twist:2.9,scale:1.08,bloom:1.2,vignette:.08,exposure:1.34,pulse:.82,texScale:8.6,texSpeed:1.15,texWarp:2.9,texContrast:2.7,texKaleido:13,texMix:1,roughness:.12},
  {shapeMode:2,color:"#ffffff",accent:"#ff4d2e",twist:1.8,scale:1.22,bloom:1.35,vignette:.12,exposure:1.38,pulse:1,texScale:6.2,texSpeed:.9,texWarp:2.45,texContrast:2.1,texKaleido:11,texMix:.96,roughness:.08}
];
function showParameters(){const result={...state.params};if(state.showcase){const act=showActs[Math.floor((state.time%60)/10)],mix=state.params.showMix??1,beat=.5+.5*Math.sin(state.time*Math.PI*2*(state.bpm/60));for(const [key,value] of Object.entries(act))result[key]=typeof value==="number"&&typeof state.params[key]==="number"?state.params[key]*(1-mix)+value*mix:mix>.5?value:state.params[key];result.bloom+=beat*.16*mix;result.pulse=Math.min(1,result.pulse+beat*.08*mix);result.twist+=Math.sin(state.time*.7)*.18*mix;}return applyParameterBindings(result);}
function updateSceneOverlays(){const root=$("#sceneOverlay");if(!root)return;const texts=state.nodes.filter(node=>node.kind==="Text"&&node.enabled),defs=Object.fromEntries(paramDefinitions.Text.map(([,key,min,max,,fallback])=>[key,[min,max,fallback]]));while(root.children.length>texts.length)root.lastElementChild.remove();texts.forEach((node,index)=>{node.params??={};const value=key=>{const [min,max,fallback]=defs[key];return boundValue(node,key,node.params[key]??fallback,min,max);},element=root.children[index]||root.appendChild(document.createElement("div")),sequence=node.params.sequence;element.className="scene-text";element.textContent=sequence?.length?sequence[Math.floor((state.time%60)/10)%sequence.length]:node.params.text||"WERKKZEUG";element.style.setProperty("--text-x",`${value("textX")}%`);element.style.setProperty("--text-y",`${value("textY")}%`);element.style.setProperty("--text-size",`${value("textSize")}px`);element.style.setProperty("--text-color",node.params.textColor||state.params.accent);element.style.setProperty("--text-rotation",`${value("textRotation")+Math.sin(state.time*.8)*2}deg`);element.style.setProperty("--text-wave",`${Math.sin(state.time*2+index)*12*value("textWave")}px`);});}
...
function initTimeline(){
  ...
  $("#ruler").innerHTML=Array.from({length:13},(_,i)=>`<span>${String(state.showcase?i*5:i).padStart(2,"0")}</span>`).join("");
...
function scheduleStep(step,time){
  const stepDuration=60/state.bpm/4,seconds=step*stepDuration%state.duration,section=Math.min(5,Math.floor(seconds/10)),s=step%16,bar=Math.floor(step/16),kickPatterns=[[0,7,8,11],[0,3,7,8,11,14],[0,6,8,10,14],[0,8,11],[0,3,6,8,10,13,15],[0,2,4,7,8,10,12,14]][section];
...
function frame(now){
  ...
  updateSceneOverlays();updateBindingReadouts();updateBoundMasterGain();scheduleAudio();updateAudioMeter();
  ...
  else if(glState){const show=showParameters(),{gl,prog,uniform}=glState,c=gl.canvas,dpr=Math.min(devicePixelRatio,2),w=Math.floor(c.clientWidth*dpr),h=Math.floor(c.clientHeight*dpr);if(c.width!==w||c.height!==h){c.width=w;c.height=h;gl.viewport(0,0,w,h);$("#resolution").textContent=`${w} × ${h}`;}gl.useProgram(prog);gl.uniform2f(uniform("r"),w,h);gl.uniform1f(uniform("t"),state.time);gl.uniform1f(uniform("yaw"),boundSpecial("Camera","cameraYaw",orbit.yaw)+(state.showcase?state.time*.16:0));gl.uniform1f(uniform("pitch"),boundSpecial("Camera","cameraPitch",orbit.pitch)+(state.showcase?Math.sin(state.time*.23)*.16:0));gl.uniform1f(uniform("dist"),boundSpecial("Camera","cameraDistance",orbit.distance)+(state.showcase?Math.sin(state.time*.31)*.35:0));for(const [u,k] of [["shapeMode","shapeMode"],["twist","twist"],["bloom","bloom"],["vignette","vignette"],["pulse","pulse"],["scale","scale"],["rough","roughness"],["exposure","exposure"],["texScale","texScale"],["texSpeed","texSpeed"],["texWarp","texWarp"],["texContrast","texContrast"],["texKaleido","texKaleido"],["texMix","texMix"]])gl.uniform1f(uniform(u),show[k]);gl.uniform1f(uniform("textureView"),state.graphMode==="textures"?1:0);const hex=show.color.match(/\w\w/g).map(x=>parseInt(x,16)/255),accent=show.accent.match(/\w\w/g).map(x=>parseInt(x,16)/255);gl.uniform3f(uniform("color"),...hex);gl.uniform3f(uniform("accent"),...accent);gl.drawArrays(gl.TRIANGLES,0,3);}
...
function bindUI(){
  ...
  window.onresize=drawWires;document.onkeydown=e=>{lastExternalEvent=performance.now()/1000;if(e.target.matches("input,select,textarea"))return;if(e.code==="Space"){e.preventDefault();togglePlay()}if(e.key.toLowerCase()==="m")$("#audioToggle").click();if(e.key==="Delete"||e.key==="Backspace"){e.preventDefault();removeSelected();}if(e.key.toLowerCase()==="f")setZoom(1);if((e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==="s"){e.preventDefault();autosave()}};
  updateAudioUI();
}
