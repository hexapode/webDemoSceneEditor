...
const defaultNodes = [
  { id:"TXT_00", name:"Hahahack Titles", type:"TEXT OPERATOR", kind:"Text", x:35,y:82,color:"#ff3b8d", value:"6 ACTS", enabled:true,params:{text:"HAHAHACK",textColor:"#fff3b0",textSize:82,textX:50,textY:48,textRotation:-2,textWave:.42,sequence:["HAHAHACK","ABSURD MACHINE","CODEX MADE THIS","MAKE IT WEIRD","SHIP THE CHAOS","HAHAHACK!"]} },
...
const defaults = { shapeMode:0,color:"#ff3b8d", accent:"#5cf5ff", metallic:0.91, roughness:0.18, twist:1.4, bloom:0.92, vignette:.2, pulse:0.72, scale:1, exposure:1.2, texScale:4.8, texSpeed:.52, texWarp:1.65, texContrast:1.8, texKaleido:7, texMix:.86, audioCutoff:1320, audioDecay:0.3, audioDelay:0.22, audioDrive:0.64 };
const DEFAULT_SCENE_VERSION=2;
...
const paramDefinitions = {
  ...
  Camera:[["Distance","cameraDistance",2.5,8,.05,4.5],["Yaw","cameraYaw",-3.14,3.14,.01,.15],["Pitch","cameraPitch",-1.2,1.2,.01,.1]],Output:[["Exposure","exposure",.2,2,.01],["Bloom","bloom",0,1.5,.01]],"Scene Input":[["Exposure","exposure",.2,2,.01]]
};
...
function showParameters(){if(!state.showcase)return state.params;const act=showActs[Math.floor((state.time%60)/10)],beat=.5+.5*Math.sin(state.time*Math.PI*2*(state.bpm/60));return {...state.params,...act,bloom:act.bloom+beat*.16,pulse:Math.min(1,act.pulse+beat*.08),twist:act.twist+Math.sin(state.time*.7)*.18};}
function updateSceneOverlays(){const root=$("#sceneOverlay");if(!root)return;const texts=state.nodes.filter(node=>node.kind==="Text"&&node.enabled);while(root.children.length>texts.length)root.lastElementChild.remove();texts.forEach((node,index)=>{node.params??={};const element=root.children[index]||root.appendChild(document.createElement("div")),sequence=node.params.sequence;element.className="scene-text";element.textContent=sequence?.length?sequence[Math.floor((state.time%60)/10)%sequence.length]:node.params.text||"WERKKZEUG";element.style.setProperty("--text-x",`${node.params.textX??50}%`);element.style.setProperty("--text-y",`${node.params.textY??50}%`);element.style.setProperty("--text-size",`${node.params.textSize??64}px`);element.style.setProperty("--text-color",node.params.textColor||state.params.accent);element.style.setProperty("--text-rotation",`${(node.params.textRotation??0)+Math.sin(state.time*.8)*2}deg`);element.style.setProperty("--text-wave",`${Math.sin(state.time*2+index)*12*(node.params.textWave??.25)}px`);});}
function renderInspector(){
  const n=allNodes().find(x=>x.id===state.selected)||currentNodes()[0];if(!n)return;$("#nodeId").textContent=n.id;$("#inspectorName").textContent=n.name;$("#inspectorType").textContent=n.type;$("#enableNode").classList.toggle("active",n.enabled);
  ...
  if($("#baseColor"))$("#baseColor").oninput=e=>{state.params.color=e.target.value;e.target.nextElementSibling.value=e.target.value.toUpperCase();autosave();};if($("#accentColor"))$("#accentColor").oninput=e=>{state.params.accent=e.target.value;e.target.nextElementSibling.value=e.target.value.toUpperCase();autosave();};if($("#textColor"))$("#textColor").oninput=e=>{n.params??={};n.params.textColor=e.target.value;e.target.nextElementSibling.value=e.target.value.toUpperCase();updateSceneOverlays();autosave();};if($("[data-text-param]"))$("[data-text-param]").oninput=e=>{n.params??={};n.params[e.target.dataset.textParam]=e.target.value;updateSceneOverlays();autosave();};$$('[data-param]').forEach(range=>range.oninput=e=>{const key=e.target.dataset.param,value=Number(e.target.value);setParameterValue(n,key,value);$(`[data-value="${key}"]`).value=value.toFixed(Number(e.target.step)>=1?0:2);updateSceneOverlays();autosave();});
}
...
function bass(time,step,midi){const notes=[36,36,43,36,39,36,46,43],note=midi??notes[Math.floor(step/2)%notes.length],freq=440*Math.pow(2,(note-69)/12),ctx=audioEngine.ctx,filter=ctx.createBiquadFilter();filter.type="lowpass";filter.frequency.setValueAtTime(state.params.audioCutoff,time);filter.frequency.exponentialRampToValueAtTime(Math.max(120,state.params.audioCutoff*.35),time+.22);filter.Q.value=4+state.params.audioDrive*10;tone(time,freq,.24+state.params.audioDecay*.3,.13,"sawtooth",filter);filter.connect(audioEngine.master);}
...
function frame(now){
  ...
  updateSceneOverlays();scheduleAudio();updateAudioMeter();
...
function bindUI(){
  ...
  $("#enableNode").onclick=()=>{const n=allNodes().find(x=>x.id===state.selected);n.enabled=!n.enabled;renderGraph();renderInspector();updateSceneOverlays();autosave()};
